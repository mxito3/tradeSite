...
class PayController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			if (isset($_POST['alipay'])) {
				...
				$mo->execute('lock tables zhisucom_user_coin write,zhisucom_mycz write,zhisucom_finance write');
				$rs = array();
				...
				$rs[] = $mo->table('zhisucom_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function movepay()
	{
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables zhisucom_user_coin write,zhisucom_mycz write');
			$rs = array();
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = $mo->table('zhisucom_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
