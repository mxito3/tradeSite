...
class ChartController extends HomeController
{
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from zhisucom_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
