...
class BazaarController extends HomeController
{
	...
	public function mywtUp($market, $num, $price, $paypassword)
	{
		...
		$rs[] = $mo->table('zhisucom_bazaar')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_config['fee'], 'addtime' => time(), 'status' => 0));
	...
	public function buy($id = NULL, $num = NULL, $paypassword = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_bazaar write , zhisucom_bazaar_log write');
		$rs = array();
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user['id']))->setDec($rmb, $mum);
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $bazaar['userid']))->setInc($rmb, round($mum - $fee, 8));
		$rs[] = $mo->table('zhisucom_bazaar_log')->add(array('userid' => $user['id'], 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'addtime' => time(), 'status' => 1));
		$rs[] = $mo->table('zhisucom_bazaar_log')->add(array('userid' => 0, 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $fee, 'fee' => 0, 'addtime' => time(), 'status' => 1));
