		...
		if (!userid()) {
			...
			if($data['awardStatus']==3){
				...
				if($prize_id>0 && $prize_status==0){
					...
					$mo->execute('lock tables zhisucom_user_award write,zhisucom_user write');
					$rs[] = $mo->table('zhisucom_user_award')->add($awardInfo);
					$rs[] = $mo->table('zhisucom_user')->save($uinfo);
					$flag = true;
					if ($rs) {
						if ($flag) {
							$mo->execute('commit');
					...
					$mo = M();
					$mo->execute('set autocommit=0');
					$mo->execute('lock tables zhisucom_user write');
					$rs = $mo->table('zhisucom_user')->save($uinfo);
					$flag = true;
					if ($rs) {
						if ($flag) {
