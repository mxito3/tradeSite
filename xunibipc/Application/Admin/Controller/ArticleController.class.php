	...
	public function checkUpdata()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('footer', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article` ADD COLUMN `footer` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			if (!in_array('index', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article` ADD COLUMN `index` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			$DbFields = M('ArticleType')->getDbFields();

			if (!in_array('footer', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article_type` ADD COLUMN `footer` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			if (!in_array('index', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article_type` ADD COLUMN `index` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			if (!in_array('content', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article_type` ADD COLUMN `content` TEXT NOT NULL    COMMENT \' \' AFTER `id`;');
			}

			if (!in_array('shang', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_article_type` ADD COLUMN `shang` TEXT NOT NULL    COMMENT \' \' AFTER `id`;');
			}
