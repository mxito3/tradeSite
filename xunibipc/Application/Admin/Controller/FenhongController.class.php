...
class FenhongController extends AdminController
{
	...
	public function fenfa($id = NULL, $fid = NULL, $dange = NULL)
	{
		...
		if ((0 < $id) && ($id <= S('fenhong_fenfa_c'))) {
			...
			$mo->execute('lock tables zhisucom_user_coin write,zhisucom_fenhong_log write');
			$rs = array();
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $fenfa[$id]['userid']))->setInc($fenhong['coinjian'], $faduoshao);
			$rs[] = $mo->table('zhisucom_fenhong_log')->add(array('name' => $fenhong['name'], 'userid' => $fenfa[$id]['userid'], 'coinname' => $fenhong['coinname'], 'coinjian' => $fenhong['coinjian'], 'fenzong' => $fenhong['num'], 'price' => $dange, 'num' => $fenfa[$id]['chiyou'], 'mum' => $faduoshao, 'addtime' => time(), 'status' => 1));
