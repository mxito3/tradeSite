...
class HuafeiController extends AdminController
{
	...
	public function repeal($id = NULL)
	{
		...
		$rs = array();
		$user_coin = $mo->table('zhisucom_user_coin')->where(array('userid' => $huafei['userid']))->find();
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $huafei['userid']))->setInc($huafei['type'], $huafei['mum']);
		$rs[] = $mo->table('zhisucom_huafei')->where(array('id' => $id))->save(array('endtime' => time(), 'status' => 2));
	...
	public function config()
	{
		...
		if (!in_array('huafei_appkey', $Config_DbFields)) {
			M()->execute('ALTER TABLE `zhisucom_config` ADD COLUMN `huafei_appkey` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_openid', $Config_DbFields)) {
			M()->execute('ALTER TABLE `zhisucom_config` ADD COLUMN `huafei_openid` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_zidong', $Config_DbFields)) {
			M()->execute('ALTER TABLE `zhisucom_config` ADD COLUMN `huafei_zidong` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}
