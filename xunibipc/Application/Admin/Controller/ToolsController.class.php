	...
	public function checkUpdata()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('class', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_version_game` ADD COLUMN `class` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
			}

			if (!in_array('shuoming', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_version_game` ADD COLUMN `shuoming` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
			}

			if (!in_array('gongsi', $DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_version_game` ADD COLUMN `gongsi` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
			}
