...
class ShopController extends AdminController
{
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables zhisucom_user_coin write,zhisucom_shop_log write');
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('zhisucom_shop_log')->where(array('id' => $id))->save(array('status' => 2));
