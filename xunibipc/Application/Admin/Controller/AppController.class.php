...
class AppController extends AdminController
{
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('zhisucom_user ON zhisucom_user.id = zhisucom_app_vipuser.uid')->join('zhisucom_app_vip ON zhisucom_app_vip.id = zhisucom_app_vipuser.vip_id')->field('zhisucom_user.username,zhisucom_app_vipuser.*,zhisucom_app_vip.name as vip_name,zhisucom_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('zhisucom_user ON zhisucom_user.id = zhisucom_app_vipuser.uid')->join('zhisucom_app_vip ON zhisucom_app_vip.id = zhisucom_app_vipuser.vip_id')->field('zhisucom_user.username,zhisucom_app_vipuser.*,zhisucom_app_vip.name as vip_name,zhisucom_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
