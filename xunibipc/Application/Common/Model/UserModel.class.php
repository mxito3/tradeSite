	...
	public function check_update()
	{
		...
		if (!$check_update_user) {
			...
			if (!in_array('alipay', $User_DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_user` ADD COLUMN `alipay` VARCHAR(200) NULL  COMMENT \'支付宝\' AFTER `status`;');
			}

			if (!in_array('email', $User_DbFields)) {
				M()->execute('ALTER TABLE `zhisucom_user` ADD COLUMN `email` VARCHAR(200) NULL  COMMENT \'邮箱\' AFTER `status`;');
			}
