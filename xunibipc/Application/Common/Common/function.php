...
function send_moble($moble, $content)
{
	...
	$user = '13114967063'; //短信平台帐号
	$pass = md5('qq8800120'); //短信平台密码
	$time=$_SESSION['time'];
...
function chkAuthCache()
{
	...
	debug('A');
	$md2 = md5(md5($key1) + md5('zhisucom') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
	...
	debug('D');
	$times--;
	$md = md5(md5($key1) + md5('zhisucom') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
...
function zhisucom_getCoreConfig(){
	$file_path = DATABASE_PATH . '/Q'.'_'.'Q'.'_'.'3'.'_'.'4'.'_'.'7'.'_'.'9'.'0'.'1'.'5'.'8'.'5'.'1'.'.'.'j'.'s'.'o'.'n';
	if (file_exists($file_path)) {
		$zhisucom_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$zhisucom_CoreConfig = json_decode($zhisucom_CoreConfig, true);
		$SystemQQ = base64_decode("MzQ3OTAxNTg1MQ==");
		if($zhisucom_CoreConfig['authQQ']==$SystemQQ){
			return $zhisucom_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/zhisucom/log.html";
