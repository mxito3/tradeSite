...
class MoneyController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_money_log  write , zhisucom_money  write');
			$rs = array();
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($money['coinname'], $num);
			$rs[] = $mo->table('zhisucom_money_log')->add(array('userid' => userid(), 'moneyid' => $money['id'], 'name' => $money['name'], 'num' => $num, 'tian' => $money['tian'], 'fee' => $money['fee'], 'feecoin' => $money['feecoin'], 'addtime' => time(), 'status' => 0));
			$rs[] = $mo->table('zhisucom_money')->where(array('id' => $id))->setInc('deal', $num);

			if ($money['num'] <= $money['deal']) {
				$rs[] = $mo->table('zhisucom_money')->where(array('id' => $id))->setField('status', 0);
			}
		...
		foreach ($MoneyList as $money) {
			...
			if ($money['endtime'] < $money['lasttime']) {
				...
				if ($MoneyLogList) {
					...
					foreach ($MoneyLogList as $user_money_list) {
						...
						$mo->execute('set autocommit=0');
						$mo->execute('lock tables zhisucom_user_coin write,zhisucom_money_log  write,zhisucom_money_dlog  write');
						$rs = array();
						$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user_money_list['userid']))->setInc($money['coinname'], $user_money_list['num']);
						$rs[] = $mo->table('zhisucom_money_log')->save(array('id' => $user_money_list['id'], 'status' => 0));
						$rs[] = $mo->table('zhisucom_money_dlog')->add(array('userid' => $user_money_list['userid'], 'money_id' => $money['id'], 'type' => 1, 'num' => $user_money_list['num'], 'addtime' => time(), 'content' => '理财结束,退回本金:' . $user_money_list['num'] . '个'));
			...
			if (!$money['lasttime'] || (($money['lasttime'] + $money['step']) < time())) {
				...
				foreach ($MoneyLogList as $MoneyLog) {
					...
					$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $MoneyLog['userid']))->setInc($money['feecoin'], $fee);
					...
					$rs[] = $mo->table('zhisucom_money_log')->save($MoneyLog);
					echo 'add ' . $MoneyLog['userid'] . ' dlog ' . $br;
					$rs[] = $mo->table('zhisucom_money_dlog')->add(array('userid' => $MoneyLog['userid'], 'money_id' => $money['id'], 'type' => 1, 'num' => $fee, 'addtime' => time(), 'content' => '本金:' . $money['coinname'] . ' :' . $MoneyLog['num'] . '个,获取理财利息' . $money['feecoin'] . ' ' . $fee . '个'));
	...
	public function beforeGet($id)
	{
		...
		if ($Money['coinname'] != $Money['feecoin']) {
			...
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($Money['feecoin'], $fee);
			debug($mo->table('zhisucom_user_coin')->getLastSql(), 'zhisucom_user_coin_sql0');
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setInc($Money['coinname'], $num);
			debug($mo->table('zhisucom_user_coin')->getLastSql(), 'zhisucom_user_coin_sql1');
		}
		else {
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setInc($Money['coinname'], round($num - $fee, 8));
			debug($mo->table('zhisucom_user_coin')->getLastSql(), 'zhisucom_user_coin_sql2');
		}

		$rs[] = $mo->table('zhisucom_money_log')->where(array('id' => $MoneyLog['id']))->setField('status', 0);
		debug($mo->table('zhisucom_money_log')->getLastSql(), 'zhisucom_money_log_sql');
		$rs[] = $mo->table('zhisucom_money_dlog')->add(array('userid' => userid(), 'money_id' => $Money['id'], 'type' => 2, 'num' => $fee, 'addtime' => time(), 'content' => '提前抽取' . $Money['title'] . ' 理财本金' . $Money['coinname'] . ' ' . $MoneyLog['num'] . '个,扣除利息' . $Money['feecoin'] . ': ' . $fee . '个'));
