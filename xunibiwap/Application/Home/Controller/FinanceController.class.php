...
/* 				if ($Market[$v['name'] . '_cny']['new_price']) {
	...
	public function myczm($status = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function mycz($status = NULL)
	{
		...
		$myczType_weixin = M('MyczType')->field('min')->where(array('status' => 1,'name'=>'alipay'))->find();
		$myczType_alipay = M('MyczType')->field('min')->where(array('status' => 1,'name'=>'alipay'))->find();
		$this->assign('weixinmin', $myczType_weixin['min']);
		$this->assign('alipaymin', $myczType_alipay['min']);
		
		$user_arr = M('User')->field("moble")->where(array('id' => userid()))->find();
		$this->assign('moble',$user_arr["moble"]);
		$this->assign('list', $list);
		$this->assign('page', $show);
		$this->display();
	}
	...
	public function myjl($status = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function mycz_old($status = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
...
//		}
        ...
        if ($mycz) {
			$content = '您有新的充值订单：'.$tradeno.'，金额：'.$num.'元，充值方式：'.$type.'。';
			//send_moble('15232609806', $content);
    ...
    public function myczUpo($type, $num)
    {
        if (!userid()) {
            $this->error('请先登录！');
        }
        ...
        $mycz = M('Mycz')->add(array('userid' => userid(), 'num' => $num, 'type' => $type, 'tradeno' => $tradeno, 'addtime' => time(), 'status' => 0));
        if ($mycz) {
            $this->success('充值订单创建成功！', array('uid' => userid(),'uname' => username(),'num' => $num,'tradeno' => $tradeno));
        }
        else {
            $this->error('提现订单创建失败！');
        }
	...
	public function mytx($status = NULL)
	{
		...
		if ($moble) {
			...
			$this->error('请先认证手机！', '../User/moble.html');
		}
	...
	public function mytxjl($status = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
	...
	public function mytxChexiao($id)
	{
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
		...
		$finance_hash = md5($mytx['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mytx['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.btchanges.com');
		$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
	...
	public function myzrdetails($coin = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function myzcdetails($coin = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function getLazywt() {
		...
		$Page->parameter .= 'type=' . $type . '&status=' . $status . '&market=' . $market . '&';
		$show = $Page->show();
		$data = $Moble->where($where)->order('id desc')->limit($limit.','.$pageSize)->select();
		if (!empty($data) && is_array($data)) {
			$str = '';
            foreach ($data AS $key => $var) {
				$str .= '<div class="wallet_box">';
				$str .= '<div class="margin_lbian">';
				$str .= '<div class="wallet_box1 withdraw_box1 po_re">';
				$str .= '<div class="withdraw_box1_l left">';
				$str .= '<p class="hui_d">委托价格：￥'.($var['price']*1).'</p>';
				$str .= '<p class="hui_d">'.date("m-d H:i:s",$var['addtime']).'</p>';
				$str .= '</div>';
				$str .= '<span class="right orange">';
				if($var['type']==1){
					$str .= '买入';
				}else{
					$str .= '卖出';
				...
				$str .= '</span>';
				$str .= '</div>';
				$str .= '<div class="wallet_box2 withdraw_box2">';
				$str .= '<ul>';
				$str .= '<li><span class="hui_d fz_3-6">数量：</span>'.($var['num']*1).'</li>';
				$str .= '<li><span class="hui_d fz_3-6">成交数量：</span>'.($var['deal']*1).'</li>';
				$str .= '<li><span class="hui_d">';
				if($var['status']==0){$str .= '交易中';}
				if($var['status']==1){$str .= '已完成';}
				if($var['status']==2){$str .= '已撤销';}
				$str .= '</li>';
				$str .= '</ul>';
				$str .= '</div>';
				$str .= '</div>';
	...
	public function getLazycj() {
		...
		if (!empty($data) && is_array($data)) {
            ...
            foreach ($data AS $key => $var) {
				...
				$str .= '<div class="wallet_box1 withdraw_box1 po_re">';
				$str .= '<div class="withdraw_box1_l left">';
				$str .= '<p class="hui_d">总额：￥'.($var['mum']*1).'</p>';
				$str .= '<p class="hui_d">'.date("m-d H:i:s",$var['addtime']).'</p>';
				$str .= '</div>';
				$str .= '<span class="right orange">';
				$str .= '<font class="hui_d" style="font-size:12px;">价格：</font>￥'.($var['price']*1);
				$str .= '</span>';
				$str .= '</div>';
				$str .= '<div class="wallet_box2 withdraw_box2">';
				$str .= '<ul>';
				$str .= '<li><span class="hui_d fz_3-6">类型：</span>';
				if($var['userid']==$var['peerid']){
					$str .= '<font class="buy">自买</font><font class="sell">自卖</font>';
				}else{
					if(($var['userid']==userid()) & ($var['type']==1)){
						$str .= '<font class="buy">买入</font>';
					}
					if(($var['userid']==userid()) & ($var['type']==2)){
						$str .= '<font class="buy">买入</font>';
					}
					if(($var['peerid']==userid()) & ($var['type']==1)){
						$str .= '<font class="sell">卖出</font>';
					}
					if(($var['peerid']==userid()) & ($var['type']==2)){
						$str .= '<font class="sell">卖出</font>';
					}
				}
				$str .= '</li>';
				$str .= '<li><span class="hui_d fz_3-6">数量：</span>'.($var['num']*1).'</li>';
				$str .= '<li><span class="hui_d fz_3-6">手续费：</span>￥';
	...
	public function mytj()
	{
		...
		$user_url=$this->getShort("http://".$_SERVER['HTTP_HOST']."/Login/register/invit/".userid());
	...
	public function getLazyjp() {
		...
		if (!empty($data) && is_array($data)) {
            ...
            foreach ($data AS $key => $var) {
				$invit='';
				$invit=M('User')->where(array('id' => $v['invit']))->getField('username');
				$str .= '<div class="wallet_box">';
				$str .= '<div class="margin_lbian">';
				$str .= '<div class="wallet_box1 withdraw_box1 po_re" style="height:70px;">';
				$str .= '<div class="withdraw_box1_l left">';
				$str .= '<p class="hui_d">下家用户名：'.$invit.'</p>';
				$str .= '<p class="hui_d">奖励类型：'.$var['type'].'【'.$var['name'].'】</p>';
				$str .= '<p class="hui_d">'.date("m-d H:i:s",$var['addtime']).'</p>';
				$str .= '</div>';
				$str .= '<span class="right orange" style=" line-height:70px;">';
				if($var['status']==0){$str .='未到账';}
				if($var['status']==1){$str .='已到账';}
				$str .= '</span>';
				$str .= '</div>';
				$str .= '<div class="wallet_box2 withdraw_box2">';
				$str .= '<ul>';
				$str .= '<li style="width:50%;"><span class="hui_d fz_3-6">操作金额：</span>￥'.($var['num']*1).'</li>';
				$str .= '<li style="width:50%;"><span class="hui_d fz_3-6">奖励金额：</span>￥'.($var['fee']*1).'</li>';
				$str .= '</ul>';
				$str .= '</div>';
				$str .= '</div>';
				$str .= '</div>';
