...
class HuafeiController extends HomeController
{
	public function index()
	{
		if (empty($_POST)) {
			...
			$mo->execute('lock tables  zhisucom_user_coin write  , zhisucom_huafei write ');
			$rs = array();
			...
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($type, $mum);
			$rs[] = $huafei_id = $mo->table('zhisucom_huafei')->add(array('userid' => userid(), 'moble' => $moble, 'num' => $num, 'type' => $type, 'mum' => $mum, 'addtime' => time(), 'status' => 0));

			if (C('huafei_zidong')) {
				if (huafei($moble, $num, md5($huafei_id))) {
					$rs[] = $mo->table('zhisucom_huafei')->where(array('id' => $huafei_id))->save(array('endtime' => time(), 'status' => 1));
				}
			}
	...
	public function install()
	{
		...
		if (in_array('huafei', $game_arr)) {
			...
			if (!isset($tableMap['zhisucom_huafei'])) {
				M()->execute("\r\n" . '                  CREATE TABLE  IF NOT EXISTS `zhisucom_huafei` (' . "\r\n" . '                        `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT \'自增id\',' . "\r\n" . '                        `userid` INT(11) UNSIGNED NOT NULL,' . "\r\n" . '                        `moble` VARCHAR(255) NOT NULL,' . "\r\n" . '                        `num` INT(11) UNSIGNED NOT NULL,' . "\r\n" . '                        `type` VARCHAR(50) NOT NULL,' . "\r\n" . '                        `mum` DECIMAL(20,8) NOT NULL,' . "\r\n" . '                        `addtime` INT(11) UNSIGNED NOT NULL,' . "\r\n" . '                        `endtime` INT(11) UNSIGNED NOT NULL,' . "\r\n" . '                        `status` TINYINT(4) NOT NULL COMMENT \'状态\',' . "\r\n" . '                        PRIMARY KEY (`id`),' . "\r\n" . '                        INDEX `status` (`status`)' . "\r\n" . '                    )' . "\r\n" . '                    COLLATE=\'utf8_general_ci\'' . "\r\n" . '                    ENGINE=InnoDB' . "\r\n" . '                    AUTO_INCREMENT=5' . "\r\n" . '                    ;' . "\r\n\r\n\r\n\r\n\r\n\r\n\t\t\t" . '    ');
			}

			if (!isset($tableMap['zhisucom_huafei_coin'])) {
				M()->execute("\r\n\r\n" . 'CREATE TABLE IF NOT EXISTS  `zhisucom_huafei_coin` (' . "\r\n\t" . '`id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT \'自增id\',' . "\r\n\t" . '`coinname` VARCHAR(50) NOT NULL,' . "\r\n\t" . '`price` VARCHAR(255) NOT NULL,' . "\r\n\t" . '`status` TINYINT(4)  NOT NULL COMMENT \'状态\',' . "\r\n\t" . 'PRIMARY KEY (`id`)' . "\r\n" . ')' . "\r\n" . 'COLLATE=\'gbk_chinese_ci\'' . "\r\n" . 'ENGINE=MyISAM' . "\r\n" . 'AUTO_INCREMENT=4' . "\r\n" . ';' . "\r\n" . 'INSERT INTO `zhisucom_huafei_coin` (`coinname`, `price`, `status`) VALUES (\'cny\', \'1\', 1);' . "\r\n\t\t\t" . '    ');
			}

			if (!isset($tableMap['zhisucom_huafei_type'])) {
				M()->execute("\r\n\r\n" . 'CREATE TABLE IF NOT EXISTS `zhisucom_huafei_type` (' . "\r\n\t" . '`id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT \'自增id\',' . "\r\n\t" . '`name` VARCHAR(255) NOT NULL,' . "\r\n\t" . '`title` VARCHAR(255) NOT NULL,' . "\r\n\t" . '`status` TINYINT(4)  NOT NULL COMMENT \'状态\',' . "\r\n\t" . 'PRIMARY KEY (`id`)' . "\r\n" . ')' . "\r\n" . 'COLLATE=\'gbk_chinese_ci\'' . "\r\n" . 'ENGINE=MyISAM' . "\r\n" . 'AUTO_INCREMENT=7' . "\r\n" . ';' . "\r\n\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'10\', \'10元话费充值\', 1);' . "\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'20\', \'20元话费充值\', 1);' . "\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'30\', \'30元话费充值\', 1);' . "\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'50\', \'50元话费充值\', 1);' . "\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'100\', \'100元话费充值\', 1);' . "\r\n" . 'INSERT INTO `zhisucom_huafei_type` (`name`, `title`, `status`) VALUES (\'300\', \'300元话费充值\', 1);' . "\r\n\r\n\r\n\r\n\t\t\t" . '    ');
			}
		}
		else {
			return array(0, '没有授权');
