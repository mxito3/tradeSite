...
class IssueController extends HomeController
{
	...
	public function upbuy($id, $num, $paypassword)
	{
		...
		$mo->execute('lock tables zhisucom_invit write ,  zhisucom_user_coin write  , zhisucom_issue write  , zhisucom_issue_log  write ,zhisucom_finance write');
		$rs = array();
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($Issue['buycoin'], $mum);
		$rs[] = $finance_nameid = $mo->table('zhisucom_issue_log')->add(array('userid' => userid(), 'coinname' => $Issue['coinname'], 'buycoin' => $Issue['buycoin'], 'name' => $Issue['name'], 'price' => $Issue['price'], 'num' => $num, 'mum' => $mum, 'ci' => $Issue['ci'], 'jian' => $Issue['jian'], 'unlock' => 1, 'addtime' => time(), 'endtime' => time(), 'status' => $Issue['ci'] == 1 ? 1 : 0));
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setInc($Issue['coinname'], $jd_num);
		$rs[] = $mo->table('zhisucom_issue')->where(array('id' => $id))->setInc('deal', $num);

		if ($Issue['num'] <= $Issue['deal']) {
			$rs[] = $mo->table('zhisucom_issue')->where(array('id' => $id))->setField('status', 0);
		}
		...
		if ($User['invit_1'] && $Issue['invit_1']) {
			...
			if ($invit_num_1) {
				$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $User['invit_1']))->setInc($Issue['invit_coin'], $invit_num_1);
				$rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $User['invit_1'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => '一代认购赠送', 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_1, 'addtime' => time(), 'status' => 1));
			}
		}
		...
		if ($User['invit_2'] && $Issue['invit_2']) {
			...
			if ($invit_num_2) {
				$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $User['invit_2']))->setInc($Issue['invit_coin'], $invit_num_2);
				$rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $User['invit_2'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => '二代认购赠送', 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_2, 'addtime' => time(), 'status' => 1));
			}
		}
		...
		if ($User['invit_3'] && $Issue['invit_3']) {
			...
			if ($invit_num_3) {
				$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $User['invit_3']))->setInc($Issue['invit_coin'], $invit_num_3);
				$rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $User['invit_3'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => '三代认购赠送', 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_3, 'addtime' => time(), 'status' => 1));
			}
		}
	...
	public function unlock($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_issue_log write ');
		$rs = array();
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setInc($IssueLog['coinname'], $jd_num);
		$rs[] = $mo->table('zhisucom_issue_log')->where(array('id' => $IssueLog['id']))->save(array('unlock' => $IssueLog['unlock'] + 1, 'endtime' => time()));

		if ($IssueLog['ci'] <= $IssueLog['unlock'] + 1) {
			$rs[] = $mo->table('zhisucom_issue_log')->where(array('id' => $IssueLog['id']))->save(array('status' => 1));
		}
