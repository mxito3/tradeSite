...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
		...
		if (!$data) {
			...
			if ($tradeLog) {
				foreach ($tradeLog as $k => $v) {
					...
					$data['tradelog'][$k]['oktime'] = date('m-d H:i', $v['addtime']);
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		$result = M()->query('select id,price,num,deal,mum,type,fee,status,addtime from zhisucom_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
		...
		if ($userCoin) {
			...
			$data['usercoin']['allxnb'] = floatval($userCoin[$xnb]+$userCoin[$xnb . 'd']);
	...
	public function getChat($ajax = 'json')
	{
		...
		if (!$chat) {
			$chat = M('Chat')->where(array('status' => 1))->order('id desc')->limit(20)->select();
			S('getChat', $chat);
		}
