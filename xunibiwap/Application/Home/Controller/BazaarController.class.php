...
//dump($list);die;
	...
	public function sell() {
		...
		$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_bazaar write');
		$rs = array();
		$UserCoin = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->find();
		...
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
		$mum = round($num * $price, 8);
		$rs[] = $mo->table('zhisucom_bazaar')->add(array('userid' => $uid,'coin'=>$xnb, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_config['fee'], 'addtime' => time(), 'status' => 0));
	...
	public function buy($id = NULL, $num = NULL, $coin = NULL, $paypassword = NULL)
    {
        ...
        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables zhisucom_user_coin write  , zhisucom_bazaar write , zhisucom_bazaar_log write , zhisucom_invit write');

        $rs = array();
        /*执行交易*/
        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user['id']))->setDec($rmb, $total);//买家减少余额
        //$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $bazaar['userid']))->setDec($xnb, $total);//卖家减少虚拟币
        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $bazaar['userid']))->setInc($rmb, round($total - $fee, 8));//卖家增加余额
        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user['id']))->setInc($xnb, $num);//买家增加虚拟币
        /*添加记录*/
        $rs[] = $mo->table('zhisucom_bazaar_log')->add(array('userid' => $user['id'], 'peerid' => $bazaar['id'], 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $total, 'fee' => $fee, 'addtime' => time(), 'status' => 1));
        /*改变委托剩余数量*/
        $rs[] = $mo->table('zhisucom_bazaar')->where(array('id' => $id))->setInc(deal, $num);
        /*改变委托状态*/
        $bazaar_sell = M('Bazaar')->where(array('id' => $id))->find();
        if ($bazaar_sell['deal'] == $bazaar_sell['num']) {
            $sell = $mo->table('zhisucom_bazaar')->where(array('id' => $id))->save(array('status' => 1));
        }
        ...
        if ($tj_1) {
            if ($tj_1 > 0 && isset($fee_1)) {
                ...
                if ($tj_fee_1) {
                    $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $tj_1))->setInc($rmb, $tj_fee_1);//增加提成
                    $rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $tj_1, 'invit' => $user['id'], 'name' => '一代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_1, 'addtime' => time(), 'status' => 1));//增加提成记录
                }
            }
                /*二代*/
            if($tj_2){
                if ($tj_2 > 0 && isset($fee_2)) {
                    ...
                    if ($tj_fee_2) {
                        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $tj_2))->setInc($rmb, $tj_fee_2);
                        $rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $tj_2, 'invit' => $user['id'], 'name' => '二代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_2, 'addtime' => time(), 'status' => 1));
                    }
                }
                 /*三代*/
                if ($tj_3) {
                    if ($tj_3 > 0 && isset($fee_3)) {
                        ...
                        if ($tj_fee_3) {
                           $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $tj_3))->setInc($rmb, $tj_fee_3);
                           $rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $tj_3, 'invit' => $user['id'], 'name' => '三代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_3, 'addtime' => time(), 'status' => 1));
                        }
                    }
                }
	...
	public function log()
	{
		...
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables zhisucom_invit write , zhisucom_user write , zhisucom_user_coin write  , zhisucom_bazaar write  , zhisucom_bazaar_log write');
			$rs = array();

			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user['id']))->setDec(C('rmb_mr'), $mum);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $user['id']))->setInc($bazaar['coin'], $input['num']);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $bazaar['userid']))->setInc(C('rmb_mr'), $sell_mum);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $bazaar['userid']))->setDec($bazaar['coin'], $input['num']);
			$rs[] = $mo->table('zhisucom_bazaar')->where(array('id' => $bazaar['id']))->setInc('deal', $input['num']);

			if ($bazaar['num'] <= $bazaar['deal']) {
				$rs[] = $mo->table('zhisucom_bazaar')->where(array('id' => $bazaar['id']))->save(array('status' => 1));
			}

			$rs[] = $mo->table('zhisucom_bazaar_log')->add(array('userid' => $user['id'], 'peerid' => $bazaar['userid'], 'coin' => $bazaar['coin'], 'price' => $bazaar['price'], 'num' => $input['num'], 'mum' => $mum, 'fee' => $zong_fee, 'addtime' => time(), 'status' => 1));

			if ($buy_shang_mum) {
				$invit = $mo->table('zhisucom_user')->where(array('id' => $bazaar['userid']))->find();

				if ($invit['id']) {
					$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $invit['id']))->setInc(C('rmb_mr'), $buy_shang_mum);
					$rs[] = $mo->table('zhisucom_invit')->add(array('userid' => $bazaar['userid'], 'invit' => $invit['username'], 'type' => '集市赠送', 'num' => $mum, 'mum' => $buy_shang_mum, 'addtime' => time(), 'status' => 1));
				}
			}
...
//dump($list);die;
    ...
    public function mywtUp($market, $num, $price, $paypassword)
    {
        ...
        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
        $mum = round($num * $price, 8);
        $rs[] = $mo->table('zhisucom_bazaar')->add(array('userid' => $uid, 'market' => $market, 'coin'=>$xnb, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_cof['fee'], 'addtime' => time(), 'status' => 0));
