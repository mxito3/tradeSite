	...
	protected function _initialize()
	{
		...
		if(!empty(PC_URL)){
			...
			if(!preg_match('/(iphone|android|Windows\sPhone)/i', $ua)){
				...
				if (isset($_GET['invit'])) {
					$invit = $_GET['invit'];
					$user = M('User')->where(array('id' => $invit))->find();

					if ($user['invit']) {
						$zhisucom_redirect = PC_URL."?".$user['invit'];
					}else{
						$zhisucom_redirect = PC_URL;
					}
				}else{
					$zhisucom_redirect = PC_URL;
				}
		...
		C('coin_menu_zhisucom',array('CNY','BTC'));
