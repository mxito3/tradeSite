...
class ShopController extends HomeController
{
	...
	public function buyShop($id, $num, $paypassword, $type, $goods)
	{
		...
		$mo->execute('lock tables zhisucom_user_coin write,zhisucom_shop write,zhisucom_shop_log write');
		$rs = array();
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($type, $xuyao);
		$rs[] = $mo->table('zhisucom_shop')->where(array('id' => $Shop['id']))->save(array(
			'deal' => array('exp', 'deal+' . $num),
			'num'  => array('exp', 'num-' . $num)
			));

		if ($Shop['num'] - $num <= 0) {
			$rs[] = $mo->table('zhisucom_shop')->where(array('id' => $Shop['id']))->save(array('status' => 0));
		}

		$rs[] = $mo->table('zhisucom_shop_log')->add(array('userid' => userid(), 'shopid' => $Shop['id'], 'price' => $Shop['price'], 'coinname' => $type, 'xuyao' => $xuyao, 'num' => $num, 'mum' => $mum, 'addr' => $my_goods['truename'] . '|' . $my_goods['moble'] . '|' . $my_goods['addr'], 'addtime' => time(), 'status' => 0));
