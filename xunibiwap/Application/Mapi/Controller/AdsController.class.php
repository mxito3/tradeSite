...
class AdsController extends CommonController {
    ...
    public function click($hash,$aid){
        ...
        $mo->execute('lock tables zhisucom_app_log write ,zhisucom_user_coin write');

        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($price_coin['name'], $Vip['price_num']);
        $rs[] = $mo->table('zhisucom_app_log')->add(
            array(
                'uid'     => $uid,
                'type'    => 'click_ads',
