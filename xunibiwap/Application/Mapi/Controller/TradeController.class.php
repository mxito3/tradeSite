...
class TradeController extends CommonController
{
	...
	public function getTradedata($marketid = NULL)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from zhisucom_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 5;');
		$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from zhisucom_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 5;'));
	...
	public function upTrade($paypassword = NULL, $marketid = NULL, $price, $num, $type)
	{
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from zhisucom_trade where userid=' . $uid . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables zhisucom_trade write ,zhisucom_user_coin write');
		$rs = array();

		if ($type == 1) {
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($rmb, $mum);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setInc($rmb . 'd', $mum);
			$rs[] = $mo->table('zhisucom_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('zhisucom_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
	...
	public function matchingTrade($market = NULL)
	{
		...
		$mo = M();
		$mo->table('zhisucom_trade')->where('num<deal')->setField('status', 1);
		...
		for (; true; ) {
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables zhisucom_trade write ,zhisucom_trade_log write ,zhisucom_user write,zhisucom_user_coin write,zhisucom_invit write');
				...
				$rs[] = $mo->table('zhisucom_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
	...
	public function mywt($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_trade write ');
		$rs = array();

		if ($trade['type'] == 1) {
			$mun = round(((($trade['num'] - $trade['deal']) * $trade['price']) / 100) * (100 + $trade_fee), 8);
			$user_buy = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$user_sell = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->setInc($xnb, $save_sell_xnb);
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->setDec($xnb . 'd', $save_sell_xnb);
			$rs[] = $mo->table('zhisucom_trade')->where(array('id' => $trade['id']))->setField('status', 2);
			$you_sell = $mo->table('zhisucom_trade')->where(array(
				'market' => array('like', '%' . $xnb . '%'),
				'status' => 0,
			...
			if (!$you_sell) {
				$you_user_sell = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->find();

				if (0 < $you_user_sell[$xnb . 'd']) {
					$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->setField($xnb . 'd', 0);
				}
			}
		}
		else {
