...
class UserController extends CommonController
{
	...
	public function upmytx($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables zhisucom_mytx write , zhisucom_user_coin  write ');
		$rs = array();
		$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec('cny', $num);
		$rs[] = $mo->table('zhisucom_mytx')->add(array('userid' => $uid, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function upmyzc($coin_id, $moble_verify, $num, $paypassword, $addr)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
			...
			$rs[] = $mo->table('zhisucom_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('zhisucom_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$mo->execute('lock tables  zhisucom_user_coin write  , zhisucom_myzc write ,zhisucom_myzr write, zhisucom_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('zhisucom_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
				debug(array('res' => $r, 'lastsql' => $mo->table('zhisucom_user_coin')->getLastSql()), '更新用户人民币');
				$rs[] = $aid = $mo->table('zhisucom_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));
				...
				debug(array('res' => $r, 'lastsql' => $mo->table('zhisucom_myzc')->getLastSql()), '转出记录');
